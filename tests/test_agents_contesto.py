...
def test_contesto_jobinpa_con_bandi_dalla_api(conn):
    ...
    contesto = agents._contesto_jobinpa(None, limite=3, client=client)
...
def test_contesto_jobinpa_per_concorso_specifico(conn):
    ...
    contesto = agents._contesto_jobinpa("CONC-1", client=client)
    assert "Concorso di prova per 10 posti" in contesto
    assert client.chiamate == [("bando", "CONC-1")]


def test_contesto_jobinpa_client_non_configurato(conn):
    ...
    contesto = agents._contesto_jobinpa(None, limite=3)
    assert contesto == ""
